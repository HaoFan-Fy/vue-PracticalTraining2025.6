<template>
  <div class="login-change-password-container">
    <div class="login-change-password-box">
      ...
      <el-form
        ...
        :model="formData"
        :rules="formRules"
        class="login-change-password-form"
      ...
      >
        ...
        <el-form-item>
          <el-button
            type="primary"
            size="large"
            class="login-change-password-btn"
            :loading="loading"
            @click="handleSubmit"
          >
            {{ loading ? '修改中...' : '确认修改' }}
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        ...
        // 查询用户信息验证原密码
        const response = await this.$http.get(`/sys_user?user_name=${this.formData.username}`)
        const users = response.data || []
        const currentUser = users.find(user => user.user_name === this.formData.username)
...
.login-change-password-header h2 {
  color: #333;
  font-size: 28px;
  font-weight: 600;
  margin: 0 0 10px 0;
...
.login-change-password-form .el-form-item {
  margin-bottom: 20px;
}

.login-change-password-btn {
  width: 100%;
  height: 45px;
  font-size: 16px;
  font-weight: 500;
...
.login-change-password-btn:hover {
