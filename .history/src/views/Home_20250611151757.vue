  ...
  methods: {
    ...
    async loadBusinessData() {
      try {
        // 模拟从API获取业务数据
        // 实际项目中这里应该调用真实的API
        const months = generateDateLabels(12)
        this.businessData.salesData = {
          labels: months,
          datasets: [{
            label: '销售额',
            data: generateRandomData(12, 100000, 500000),
        ...
        this.businessData.revenueData = {
          ...
          datasets: [
            {
              label: '收入',
              data: generateRandomData(12, 150000, 400000),
              backgroundColor: tailwindConfig().theme.colors.green[500]
            },
            {
              label: '支出',
              data: generateRandomData(12, 100000, 300000),
        ...
        this.businessData.regionData = {
          labels: ['华东', '华南', '华北', '华中', '西南', '西北', '东北'],
          datasets: [{
            label: '客户数量',
            data: generateRandomData(7, 500, 2000),
            backgroundColor: `rgba(${hexToRGB(tailwindConfig().theme.colors.indigo[500])}, 0.8)`,
