    ...
    async loadMenuData() {
      try {
        ...
        if (response.data && response.data.length > 0) {
          // 修正API菜单数据的path字段，仅对一级目录下的二级菜单补全前缀
          const fixMenuPath = (menus, parentPath = "") => {
            menus.forEach(menu => {
              if (menu.menu_type === 'C' && menu.path && !menu.path.startsWith('/')) {
                // 只对一级目录下的二级菜单补一次前缀
                menu.path = parentPath + '/' + menu.path;
                // 防止多次补全，递归时parentPath始终为根目录
                if (menu.children && menu.children.length > 0) {
                  fixMenuPath(menu.children, menu.path);
                }
              } else if (menu.children && menu.children.length > 0) {
                fixMenuPath(menu.children, menu.path || parentPath);
              }
            });
          };
