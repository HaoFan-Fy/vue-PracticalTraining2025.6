    ...
    async loadMenuData() {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          const fixMenuPath = (menus, parentPath = "", level = 1) => {
            menus.forEach(menu => {
              // 只对一级目录下的二级菜单补一次前缀，且 path 不以 / 开头且未被补全过
              if (menu.menu_type === 'C' && menu.path && !menu.path.startsWith('/') && level === 2) {
                menu.path = parentPath + '/' + menu.path;
              }
              if (menu.children && menu.children.length > 0) {
                fixMenuPath(menu.children, menu.path || parentPath, level + 1);
              }
            });
          };
